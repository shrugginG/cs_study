...
std::pair<typename HashMap<K, M, H>::iterator, bool> HashMap<K, M, H>::insert(const value_type& value) {
    ...
    //std::shared_ptr temp = std::make_shared<node>(value, _buckets_array[index]);
    //auto temp = new node(value, _buckets_array[index]);
    //_buckets_array[index] = temp;
    _buckets_array[index] = std::move(std::make_shared<node>(value, _buckets_array[index]));
...
typename HashMap<K, M, H>::node_pair HashMap<K, M, H>::find_node(const K& key) const {
    size_t index = _hash_function(key) % bucket_count();
    std::shared_ptr<node> curr = _buckets_array[index];
    std::shared_ptr<node> prev = nullptr; // if first node is the key, return {nullptr, front}
    while (curr != nullptr) {
...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::iterator HashMap<K, M, H>::make_iterator(std::shared_ptr<node> curr) {
    if (curr == nullptr) {
...
template <typename K, typename M, typename H>
    ...
    for (size_t i = 0; i < bucket_count(); ++i) {
        ...
        std::shared_ptr<node> curr = _buckets_array[i];
        while (curr != nullptr) {
...
std::vector<std::shared_ptr<node> > new_buckets_array(new_bucket_count);
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(const HashMap<K, M, H>& other) {
   ...
   _buckets_array = std::vector<std::shared_ptr<node> >{other.bucket_count()};
